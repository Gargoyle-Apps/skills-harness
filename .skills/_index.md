...
| toon-export | When encoding agent-facing structured data as TOON from JSON, YAML, CSV, or emitters — normalize to JSON, encode via official CLI, strict-decode, and run mistake checks. One skill; no per-format siblings. | toon export, encode toon, validate toon, json to toon, yaml to toon, csv to toon, toon format, write a .toon file, check toon, token efficient structured data |
